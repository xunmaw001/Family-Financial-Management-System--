<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="账单" prop="zhangdan">
          <el-input v-model="ruleForm.zhangdan" 
              placeholder="账单" clearable  :readonly="ro.zhangdan"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="账单" prop="zhangdan">
              <el-input v-model="ruleForm.zhangdan" 
                placeholder="账单" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="收支状态" prop="shouzhizhuangtai">
          <el-select v-model="ruleForm.shouzhizhuangtai" placeholder="请选择收支状态">
            <el-option
                v-for="(item,index) in shouzhizhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="收支状态" prop="shouzhizhuangtai">
	      <el-input v-model="ruleForm.shouzhizhuangtai"
                placeholder="收支状态" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='zhangdan'){
            this.ruleForm.zhangdan = obj[o];
	    this.ro.zhangdan = true;
            continue;
          }
          if(o=='zhanghao'){
	    ...
	    this.ro.shanghumingcheng = true;
          ...
          if(o=='shouzhizhuangtai'){
            this.ruleForm.shouzhizhuangtai = obj[o];
	    this.ro.shouzhizhuangtai = true;
            continue;
          }
          if(o=='jiaoyijine'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shouzhizhuangtaiOptions = "收入,支出".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `caiwumingxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `caiwumingxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.caiwumingxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
